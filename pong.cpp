...
bool Pong::init()
{
    ...
    test_image = NULL;
    main_Window = SDL_CreateWindow("Pong", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, window_width, window_height, SDL_WINDOW_SHOWN);
...
void Pong::change_resolution(string new_resolution)
{
...
void Pong::clear_screen(const SDL_Color & color)
{
    SDL_Rect temp;
...
void Pong::fps_cap(Uint32 start, Uint32 end, int frames)
{
    int delay = 1000/frames - (end - start);
    if (delay > 0) { SDL_Delay(delay); }
}

string Pong::int_to_string(int num = 0)
{
    stringstream ss;
    ss << num;
    return ss.str();
...
int Pong::game_loop()
{
    ...
    TextBox title("Pong", window_width * 4/10, window_height * 1/20, window_width * 2/10, window_height * 1/10, font[0], White, White);
...
MenuCodes Pong::main_menu()
{
    clear_screen(Gray);
    SDL_Event event;
    string font[2] = { "COMPUTERAMOK.ttf" , "Pixel Countdown.ttf"};
    TextBox title("Pong", window_width * 8/10, window_height * 1/10, window_width * 6/10, window_height * 2/10, font[0], White, White);
...
MenuCodes Pong::howtoplay_menu()
{
    ...
    while(1)
    {
        if (SDL_PollEvent(&event))
        {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN)
            {
                if (event.button.button == SDL_BUTTON_LEFT) { if (back.Detection()) { return MAIN; } }
            }
        }
        Uint32 start = SDL_GetTicks();
        title.Render(main_Renderer, Gray);
...
int Pong::color_menu()
{
    ...
    SDL_Event event;
    string font[2] = { "COMPUTERAMOK.ttf" , "Pixel Countdown.ttf"};
    TextBox title("Pong", window_width * 2/10, window_height * 1/10, window_width * 6/10, window_height * 2/10, font[0], White, White);
        ...
        if (SDL_PollEvent(&event))
        {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN)
            {
                if (event.button.button == SDL_BUTTON_LEFT)
                {
                    ...
                    else if (confirm.Detection()) { if (C1 != C2) { return 0; } }
                }
            }
        }
        Uint32 start = SDL_GetTicks();
...
Direction Pong::determine_movement(const Ball & ball, const Paddle & paddle, const int & diff)
{
